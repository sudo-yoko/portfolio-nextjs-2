...
app.post(path, async (req: Request<never, unknown, ReqBody, never>, res: Response<string>): Promise<void> => {
  console.log(logPrefix + `Request(inbound) -> prompt=${req.body.prompt}, model=${req.body.model}`);
  ...
  const interval = setInterval(() => {
    ...
    console.log(logPrefix + `Response(outbound) -> stream chunk sent: ${char}`);
...
app.listen(port, () => {
  console.log(consolePrefix + `Mock service running on http://localhost:${port}${path}`);
});
