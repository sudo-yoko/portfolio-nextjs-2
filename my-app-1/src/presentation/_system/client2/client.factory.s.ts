...
import { createAxiosClient } from '@/presentation/_system/client2/client.adapter.axios';
import { axiosInstance } from '@/presentation/_system/client2/client.core.axios';
    ...
    switch (type) {
        case 'axios-proxy': {
            ...
            const { axiosInstance: proxyInstance } = await import(
                '@/presentation/_system/client2/client.core.axios.proxy'
            );
            return createAxiosClient(proxyInstance, logger);
        }
        case 'fetch': {
            const { createFetchClient } = await import('@/presentation/_system/client2/client.adapter.fetch');
