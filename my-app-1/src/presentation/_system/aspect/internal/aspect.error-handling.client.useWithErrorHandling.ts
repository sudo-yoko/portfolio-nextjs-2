...
const logPrefix = 'aspect.error-handling.client.useWithErrorHandling.ts: ';
...
export function useWithErrorHandling() {
     ...
     */
    // TODO: onAbort引数の検討。エラー時にエラーモーダルを出したい場合の考慮
    const withErrorHandling = (subject: () => void): void => {
     ...
     */
    const withErrorHandlingAsync = async (subject: () => Promise<void>): Promise<void> => {
    ...
    return { withErrorHandling, withErrorHandlingAsync };
}
